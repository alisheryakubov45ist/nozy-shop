...
fetch("products.json")
  .then(r => r.json())
  .then(data => { products = data.products; renderProducts(products); });
...
function renderProducts(list){
  ...
  list.forEach(p=>{
    ...
    img.addEventListener("touchend", e=>{
        ...
        if(endX-startX>50){ prevImageMain(p); }
        else if(startX-endX>50){ nextImageMain(p); }
    });
...
// Главный экран свайп
function prevImageMain(p){ let imgs=p.images; let idx=imgs.indexOf(event.target.src); idx=(idx-1+imgs.length)%imgs.length; event.target.src=imgs[idx]; }
function nextImageMain(p){ let imgs=p.images; let idx=imgs.indexOf(event.target.src); idx=(idx+1)%imgs.length; event.target.src=imgs[idx]; }
...
function renderCart(){
  ...
  el.innerHTML="";
  let total=0;
  cart.forEach((i,idx)=>{ total+=i.price; el.innerHTML+=`<p>${i.name} (${i.size}, ${i.color}) – ${i.price} TJS <span style="cursor:pointer;color:#ff3b30;" onclick="removeFromCart(${idx})">❌</span></p>`; });
...
function openViewer(images){
  ...
  img.addEventListener("touchstart", e=>{ startX=e.touches[0].clientX; });
  img.addEventListener("touchend", e=>{ endX=e.changedTouches[0].clientX; if(endX-startX>50){ prevImage(); } else if(startX-endX>50){ nextImage(); } });
}
